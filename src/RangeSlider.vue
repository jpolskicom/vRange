  ...
  <div class="c-rangeSlider" ref="rangeSlider">
    ...
    <div class="c-rangeSlider__wrap">
      ...
      <div class="c-rangeSlider__steps" v-if="showSteps">
        <div
          v-for="step in params.steps"
          class="c-rangeSlider__step"
          :class="{'c-rangeSlider__step--active': params.values.includes(step)}"
          :style="{'width':getPrecentageFromValue(step)+'%'}"
          :key="`step${step}`"
        >{{step}}</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getPrecentageFromTrack(position) {
      ...
      let precentage = (100 * relativePosition) / track.width;
      return precentage;
    },
    ...
    getClosestValueFromSteps(value) {
      let step = this.params.steps.reduce((a, b) => {
    ...
    generateSteps(){
      if(this.stepsArray.length > 0) {
        this.params.steps = this.stepsArray;
          ...
          for (let index = this.range[0]; index <= this.range[1]; index = index+this.step) {
